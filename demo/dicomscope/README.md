...
3. The linkage panel resolves the study to the order by Study Instance UID
   first (`IPC-3.1`, else `ZDS-1.1`, else an ORU^R01 `OBX` carrying DCM
   110180, against (0020,000D)), Accession Number second (`OBR-18`, else
   `IPC-1.1`, against (0008,0050)). Both keys are always compared. In
   practice the accession number is the key RIS and order workflows use,
   while the study UID is exact but sometimes derived or regenerated by the
   archive, so when the two keys disagree with each other the panel says
   which hop of the workflow that points at: an accession that differs on a
   UID-linked study means the RIS will not associate them; a UID that
   differs on an accession-linked study means the archive or modality
   replaced the announced UID.
